...
namespace ultra
{
	...
	struct stream_socket
	{
		...
		stream_socket(const ultra::Address &address) : addr_len(sizeof(this->addr)), enabled(false), encrypted(false)
		{
			init_api();
			//FD_ZERO(fds);
			addr.sin_addr.s_addr = address.ip ? inet_addr(address.ip) : htonl(INADDR_ANY);
