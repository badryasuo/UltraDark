...
namespace ultra
{
	...
	inline mat4 ortho(GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat near_sue, GLfloat far_sue)
	{
		GLfloat rl = (right - left);
		GLfloat tb = (top - bottom);
		GLfloat fn = (far_sue - near_sue);
		mat4 res(GL_FLAG::zero);
		res.d[0][0] = 2 / rl;
		res.d[1][1] = 2 / tb;
		res.d[3][0] = -(right + left) / rl;
		res.d[3][1] = -(top + bottom) / tb;
		res.d[2][2] = -2 / fn;
		res.d[3][2] = -(near_sue + far_sue) / fn;
		return res;
	}
