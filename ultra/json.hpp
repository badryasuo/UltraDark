...
#include <ostream>
#include <vector>
#include <udark/algorithms/adv.hpp>
namespace u
{
	struct json
	{
		...
		static inline object parse(const std::string &json)
		{
			...
			if (!json.empty())
			{
				...
				taken = u::takewhile_addpos(json_string, u::isspace, pos);
				if (json_string.front() == '{')
				{
					...
					{
						u::skipwhile_addpos(json_string, u::isspace, pos);
						...
						if (c == '"' || c == '\'')
						{
							key = u::takebetween_addpos(json_string, c, c, pos);
							...
							if (json_string.front() == ':')
							{
								json_string.erase(0, 1);
								++pos.x;
								skipwhile_addpos(json_string, u::isspace, pos);
								c = json_string.front();
								if (u::isdigit(c))
								{
									value = takewhile_addpos(json_string, u::isdigit, pos);
									if (json_string.front() == '.')
									{
										value += '.';
										json_string.erase(0, 1);
								...
								{
									taken = takewhile_addpos(json_string, u::isalpha, pos);
								...
								{
									taken = u::takebetween_addpos(json_string, c, c, pos);
									json_string.erase(0, 2);
									value = "\"" + taken + "\"";
		...
		private:
			static inline json::array parse_array(const std::string &json_array, position pos = position())
			{
				...
				{
					skipwhile_addpos(str, u::isspace, pos);
					if (str.front() == '[' && str.back() == ']')
					{
						str.erase(0, 1);
						skipwhile_addpos(str, u::isspace, pos);
