...
namespace u
{
	inline bool isalpha(const unsigned &x)
	{
		return (x >= 'A' && x <= 'Z') || (x >= 'a' && x <= 'z');
	}
	inline bool isdigit(const unsigned &x)
	{
		return x >= '0' && x <= '9';
	}
	inline bool isalphanumeric(const unsigned & x)
	{
		return isalpha(x) || isdigit(x);
	}
	inline bool isspace(const unsigned & x)
	{
		return x == ' ' || x == '\t' || x == '\r' || x == '\n';
	}
	...
	T takebetween(T &object, Item start, Item to = start)
	{
	...
	template<typename T, typename Item>
	T takebetween(const T &object, Item start, Item to = start)
	{
	...
	/// <param name='condition'>Either you can choose a condition from the u:: namespace or use u::equals&lt;Your_Object_Type&gt;(initializer_list) or even create your own condition but make sure it returns only a boolean.</param>
	/// <summary>
