...
namespace u
{
	...
	struct datagram_packet
	{
		...
		datagram_packet(byte * bytes, u::byte * address, int port) : size(strlen(bytes)), _AddrLen(sizeof(this->_Addr))
		{
		...
		datagram_packet(data data, size_t length, u::byte * address, int port) : size(length), _AddrLen(sizeof(this->_Addr)), _Data((byte *)data)
		{
			this->_Addr.sin_family = AF_INET;
			this->_Addr.sin_addr.s_addr = address ? inet_addr(address) : INADDR_ANY;
			this->_Addr.sin_port = htons(port);
